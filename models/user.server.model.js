...
exports.getOne = function(userId, cb){
    db.getPool().query("SELECT * FROM Users WHERE user_id=?", [userId], function (err, result) {
...
exports.insert = function(username, cb){
    db.getPool().query("INSERT INTO Users (username) VALUES (?)", [username], function (err, result) {
        if (err) return cb(err);
        cb(result);
    });
};
